...
const ITEM_DATABASE = {
  sword_iron: {
    id: 'sword_iron', name: 'Espada de Hierro', type: 'weapon', rarity: 'common',
    damageCategory: 'slashing', canBlock: true, statBonus: { attack: 5 }, maxDurability: 100,
  },
  sword_tempered: {
    id: 'sword_tempered', name: 'Espada Templada', type: 'weapon', rarity: 'rare',
    damageCategory: 'slashing', canBlock: true, statBonus: { attack: 9 }, maxDurability: 120,
  },
  dagger_swift: {
    id: 'dagger_swift', name: 'Daga Veloz', type: 'weapon', rarity: 'common',
    damageCategory: 'piercing', canBlock: false, statBonus: { attack: 3, speed: 1 }, maxDurability: 90,
  },
  axe_battle: {
    id: 'axe_battle', name: 'Hacha de Batalla', type: 'weapon', rarity: 'common',
    damageCategory: 'blunt', canBlock: true, statBonus: { attack: 7 }, maxDurability: 110,
  },
  spear_guard: {
    id: 'spear_guard', name: 'Lanza de Guardia', type: 'weapon', rarity: 'common',
    damageCategory: 'thrust', canBlock: true, statBonus: { attack: 6, defense: 2 }, maxDurability: 105,
  },
  armor_leather: {
    id: 'armor_leather', name: 'Armadura de Cuero', type: 'armor', rarity: 'common',
    statBonus: { defense: 3, maxHp: 10 }, maxDurability: 80,
  },
  armor_iron: {
    id: 'armor_iron', name: 'Armadura de Hierro', type: 'armor', rarity: 'rare',
    statBonus: { defense: 6, maxHp: 20 }, maxDurability: 120,
  },
  material_wolf_pelt: {
    id: 'material_wolf_pelt', name: 'Piel de Lobo', type: 'material', rarity: 'common',
  },
  material_wolf_fang: {
    id: 'material_wolf_fang', name: 'Colmillo de Lobo', type: 'material', rarity: 'common',
  },
  potion_health_minor: {
...
function addItem(itemId, qty = 1) {
  ...
  if (full) {
    showNotification(added > 0 ? 'Inventario lleno, no se pudo recoger todo' : 'Inventario lleno', 'warning');
  }
...
function _damageEquippedSlot(slot, amount) {
  ...
  if (!wasBroken && equipped.durability <= 0) {
    ...
    showNotification(`${itemData.name} está desgastado`, 'warning');
  }
...
function initInventory() {
  document.getElementById('inventory-toggle-btn').addEventListener('touchstart', (e) => {
    e.preventDefault();
    toggleInventory();
  }, { passive: false });
...
function _onActionEquipToggle() {
  if (_pendingEquippedSlot) {
    unequipItem(_pendingEquippedSlot);
  } else if (_pendingItemId) {
    equipItem(_pendingItemId);
  }
  _closeItemActionMenu();
}
